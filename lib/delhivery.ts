...
export async function getDelhiveryTrackingDetails(waybill: string): Promise<DelhiveryTrackingDetails> {
  ...
  const scans = Array.isArray(shipment?.Scans)
    ? shipment.Scans
        ...
        .filter((scan: { status: string; location: string; date: string }) => scan.status || scan.location || scan.date)
        // These are internal manifest edits, not customer-facing movement.
        .filter((scan: { status: string; location: string; date: string }) => !/package details changed|manifest uploaded|not picked|not ready/i.test(scan.status || ""))
    : []
  const deliveredScan = [...scans].reverse().find((scan) => /delivered/i.test(scan.status || ""))
  const currentStatusText = textFrom(currentStatus?.Status) || textFrom(latestScan?.Scan)
  const deliveredAt = /delivered/i.test(currentStatusText || "")
    ? deliveredScan?.date || textFrom(currentStatus?.StatusDateTime) || findNestedText(data, ["DeliveredDate", "delivered_date", "delivered_at"])
    : deliveredScan?.date || ""
...
function buildDelhiveryPayload(order: StoreOrder, pickupLocation: string, externalOrderId?: string) {
  ...
  return {
    ...
    shipments: [{
      ...
      // Express is the default for new shipments. Saksham is explicitly kept
      // on the existing service and must not be changed.
      shipping_mode: isSakshamOrder(order) ? "Surface" : "Express",
      ...
      seller_inv: order.id,
      // Delhivery accepts centimetres. Terrace FC packaging: 8 × 7 × 1 inches.
      shipment_width: 17.78,
      shipment_height: 2.54,
      shipment_length: 20.32,
      weight: calculateOrderWeightKg(order),
    }],
  }
}

function isSakshamOrder(order: StoreOrder) {
  const name = String(order.address?.name || "").toLowerCase()
  const email = String(order.address?.email || order.customerEmail || "").toLowerCase()
  return name.includes("saksham") || email.includes("saksham")
}
