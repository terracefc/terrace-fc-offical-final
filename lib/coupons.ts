...
export const RETURN_CUSTOMER_CODE = "NEXTKICK5"
...
export const DEFAULT_COUPONS: Coupon[] = [
  {
    code: RETURN_CUSTOMER_CODE,
    discountType: "percentage",
    discountValue: 5,
    perAccountUsageLimit: 1,
    description: "Private 5% returning-customer discount.",
    isActive: true,
  },
  {
    code: WELCOME_CODE,
...
export function normalizeCoupons(value: unknown): Coupon[] {
  ...
  const normalized = value
    .filter(isCoupon)
    .filter((coupon) => !["RETURN5", "SECONDHALF5"].includes(coupon.code.trim().toUpperCase()))
    .map((coupon) => ({
      ...coupon,
