...
export function orderEmailHtml(order: StoreOrder, options: {
  label?: string
  heading?: string
  message?: string
} = {}) {
  ...
  const items = order.items.map((item) => {
    ...
    const isRetroProduct = /\bretro\b/i.test(`${item.name} ${item.season}`)
    const details = [item.description || `${item.club} ${item.season}`, `Size ${item.size}`, isRetroProduct ? "" : item.version ? formatEmailVersion(item.version) : "", `Qty ${item.quantity}`, isRetroProduct ? "" : getItemOptionLabel(item)]
      .filter(Boolean)
      .join(" · ")
  ...
  return `
    <div style="margin:0;background:#f5f5f5;padding:24px 12px;font-family:Arial,sans-serif;color:#171717">
      <div style="max-width:620px;margin:0 auto;background:#ffffff;border:1px solid #e5e5e5;border-radius:18px;overflow:hidden">
        <div style="background:#171717;color:#ffffff;padding:22px 24px">
          ${brandLogoHtml({ dark: true })}
          <div style="font-size:12px;text-transform:uppercase;letter-spacing:1.6px;color:#cfcfcf;margin-top:6px">${escapeHtml(options.label || "Order Confirmation")}</div>
        </div>
        <div style="padding:24px">
          <p style="font-size:15px;line-height:1.6;margin:0 0 8px">Hi ${escapeHtml(order.address.name || "there")},</p>
          <h1 style="font-size:28px;line-height:1.1;margin:0 0 10px">${escapeHtml(options.heading || "Thank you for your purchase.")}</h1>
          <p style="font-size:15px;line-height:1.6;margin:0 0 22px;color:#444">${escapeHtml(options.message || `Your order is confirmed. Thank you for your purchase of ${orderedJerseys}. We’re getting your jersey ready, and your PDF invoice is attached for your records.`)}</p>
...
function getOrderItemImageUrl(item: StoreOrder["items"][number]) {
  const matchingKit = kits.find((kit) => kit.id === item.id || kit.id === item.id - 10000 || kit.name === item.name)
  // Prefer a catalog image when an old order stored a stale/local image path.
  const image = matchingKit?.image || item.image || "/placeholder.svg"
