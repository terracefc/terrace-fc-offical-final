...
function getItemOptions(item: StoreOrder["items"][number]) {
  ...
  if (customization?.mode === "original") {
    options.push("Original player name")
  } else if (customization?.enabled || customization?.mode === "custom") {
    options.push(`${customization.name || "Custom"} ${customization.number || ""}`.trim())
  }

  if (customization?.patches) {
    options.push(`${customization.patchType || "Sleeve"} patches`)
  }
