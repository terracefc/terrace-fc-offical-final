...
export function orderStatusEmailHtml(order: StoreOrder, options: { awbChanged?: boolean } = {}) {
  const heading = options.awbChanged
    ? `AWB number changed: ${order.trackingNumber || order.delhiveryWaybill || order.shippingId || "updated"}`
    : `${order.address.name || "Your"} order progress: ${getOrderStatusLabel(order.fulfillmentStatus)}`
  return orderEmailHtml(order, {
    label: "Order Status Update",
    heading,
    message: options.awbChanged
      ? "Your shipment has been recreated with Delhivery. The updated AWB is shown below and can be used to track your order."
      : getOrderStatusMessage(order.fulfillmentStatus),
  })
  /* legacy status-only layout retained below for reference */
  /*
