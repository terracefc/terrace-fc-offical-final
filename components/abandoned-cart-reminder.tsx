      ...
      const response = await fetch("/api/customer/cart-reminder", {
        ...
        body: JSON.stringify({
          ...
          items: cart.map((item) => ({
            ...
            version: isRetroJersey(item.kit) ? "" : getJerseyVersionLabel(item.version),
            backPrint: isRetroJersey(item.kit) || item.version === "embroidery" ? "" : getJerseyBackLabel(item.customization),
          })),
        }),
      }).catch(() => null)
