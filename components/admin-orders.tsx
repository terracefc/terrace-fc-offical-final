...
import { getOrderStatusLabel as getFulfillmentStatusLabel, getTrackingSubPhases } from "@/lib/tracking"
...
export function AdminOrders() {
  ...
  const printSelectedStickers = () => {
    ...
    printOrderStickers(selectedOrders)
  }
...
function OrderCard({
  ...
  const adminSubPhases = getTrackingSubPhases(fulfillmentStatus, order.delhiveryScans || [], order.delhiveryStatus || order.shiprocketStatus)
  const adminDelhiveryScans = (order.delhiveryScans || []).slice().reverse()
  const latestAdminDelhiveryScan = adminDelhiveryScans[0]
  const totalUnits = order.items.reduce((sum, item) => sum + item.quantity, 0)
  const parcelWeightKg = calculateOrderWeightKg(order)
  const showTrackingControls = Boolean(pendingTrackingStatus) || ["picked_up", "shipped", "out_for_delivery", "delivered"].includes(fulfillmentStatus)
  ...
  const setFulfillment = async (nextStatus: NonNullable<StoreOrder["fulfillmentStatus"]>) => {
    ...
    try {
      const shouldEmailCustomer = ["picked_up", "shipped", "out_for_delivery", "delivered"].includes(nextStatus)
  ...
  const handleCancelOrder = async () => {
    ...
    if (!reason) {
      setShippingMessage("Add a cancellation reason before cancelling this order.")
      return
    }
    ...
    <div id={`admin-order-${order.id}`} className={`rounded-xl border bg-background ${isSelected ? "border-accent ring-2 ring-accent/20" : "border-border"}`}>
      <div className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          {latestAdminDelhiveryScan && (
            <p className="mt-2 rounded-lg border border-accent/20 bg-accent/5 px-2 py-1 text-xs font-bold text-accent">
              Latest Delhivery: {latestAdminDelhiveryScan.status}
              {latestAdminDelhiveryScan.location ? ` - ${latestAdminDelhiveryScan.location}` : ""}
              {latestAdminDelhiveryScan.date ? ` - ${new Date(latestAdminDelhiveryScan.date).toLocaleString("en-IN")}` : ""}
            </p>
          )}
        </div>
      ...
      <div className="mt-4 rounded-lg border border-border/60 p-3">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <StepButton
                active={fulfillmentStatus === "confirmed"}
              ...
              />
              <StepButton
                active={fulfillmentStatus === "packaged"}
                icon={<PackageCheck className="w-4 h-4" />}
                label="Packed"
              ...
              />
              <StepButton
                active={fulfillmentStatus === "picked_up"}
                icon={<Truck className="w-4 h-4" />}
                label="Picked Up"
                onClick={() => openTrackingFields("picked_up")}
              ...
              />
              <StepButton
                active={fulfillmentStatus === "out_for_delivery"}
                icon={<Truck className="w-4 h-4" />}
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
            <label className="space-y-1">
              <span className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground">AWB / Tracking Number</span>
              <input
                value={trackingNumber}
                onChange={(event) => setTrackingNumber(event.target.value)}
                placeholder="AWB / India Post ID"
                disabled={fulfillmentStatus === "cancelled"}
                className="h-10 w-full rounded-lg border border-border bg-secondary/30 px-3 text-sm font-bold outline-none focus:border-accent sm:w-56 disabled:opacity-50"
              />
            </label>
            <label className="space-y-1">
              <span className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground">Courier</span>
              <select
                value={courierName}
                onChange={(event) => setCourierName(event.target.value)}
                disabled={fulfillmentStatus === "cancelled"}
                className="h-10 w-full rounded-lg border border-border bg-secondary/30 px-3 text-sm font-bold outline-none focus:border-accent sm:w-44 disabled:opacity-50"
              >
            ...
            <label className="space-y-1">
              <span className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground">Estimated Delivery</span>
              <input
                value={estimatedDelivery}
                onChange={(event) => setEstimatedDelivery(event.target.value)}
                placeholder="e.g. 28 May"
                disabled={fulfillmentStatus === "cancelled"}
                className="h-10 w-full rounded-lg border border-border bg-secondary/30 px-3 text-sm font-bold outline-none focus:border-accent sm:w-40 disabled:opacity-50"
              />
            </label>
            <button
              type="button"
              disabled={fulfillmentStatus === "cancelled" || isSendingShippingEmail}
              onClick={() => setFulfillment(trackingSaveStatus)}
              className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-foreground px-3 text-xs font-black uppercase tracking-widest text-background disabled:cursor-not-allowed disabled:opacity-50"
            >
              <Send className="w-4 h-4" />
              Save {getFulfillmentStatusLabel(trackingSaveStatus)}
            </button>
          </div>
          )}
        </div>
        ...
        {(order.trackingNumber || order.shippingId || order.courierName) && (
          <p className="mt-2 text-xs text-muted-foreground">
            Current tracking: <span className="font-black text-foreground">{order.trackingNumber || order.shippingId || "Not added"}</span>
            {order.courierName ? <> via <span className="font-black text-foreground">{order.courierName}</span></> : null}
          </p>
        )}
        {(order.delhiveryWaybill || order.delhiveryOrderId || order.delhiveryPickupId || adminDelhiveryScans.length > 0 || order.delhiveryError) && (
          <div className="mt-2 rounded-lg border border-border/60 bg-secondary/20 p-2 text-xs">
            <p className="font-black uppercase tracking-widest text-muted-foreground">Delhivery</p>
            {order.delhiveryWaybill && <p>Waybill: <span className="font-black">{order.delhiveryWaybill}</span></p>}
            {order.delhiveryOrderId && <p>Order ID: <span className="font-black">{order.delhiveryOrderId}</span></p>}
            {order.delhiveryPickupId && <p>Pickup ID: <span className="font-black">{order.delhiveryPickupId}</span></p>}
            {order.delhiveryStatus && <p>Status: <span className="font-black">{order.delhiveryStatus}</span></p>}
            <div className="mt-3 rounded-md border border-border bg-background/70 p-2">
              <p className="font-black uppercase tracking-widest text-muted-foreground">Add Manual Sub Phase</p>
              <div className="mt-2 grid gap-2 md:grid-cols-[1fr_1fr_180px_auto]">
                <input
                  value={manualSubPhase}
                  onChange={(event) => setManualSubPhase(event.target.value)}
                  placeholder="Out for Delivery"
                  className="h-9 rounded-lg border border-border bg-secondary/30 px-2 text-xs font-bold outline-none focus:border-accent"
                />
                <input
                  value={manualSubPhaseLocation}
                  onChange={(event) => setManualSubPhaseLocation(event.target.value)}
                  placeholder="Location"
                  className="h-9 rounded-lg border border-border bg-secondary/30 px-2 text-xs font-bold outline-none focus:border-accent"
                />
                <input
                  type="datetime-local"
                  value={manualSubPhaseDate}
                  onChange={(event) => setManualSubPhaseDate(event.target.value)}
                  className="h-9 rounded-lg border border-border bg-secondary/30 px-2 text-xs font-bold outline-none focus:border-accent"
                />
                <button
                  type="button"
                  onClick={addManualSubPhase}
                  disabled={isSendingShippingEmail}
                  className="inline-flex h-9 items-center justify-center rounded-lg border border-accent/30 px-3 text-xs font-black uppercase tracking-widest text-accent hover:bg-accent/10 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Add
                </button>
              </div>
            </div>
            {adminDelhiveryScans.length > 0 ? (
              <div className="mt-2 rounded-md border border-accent/20 bg-accent/5 p-2">
                <p className="font-black uppercase tracking-widest text-accent">Delhivery Updates</p>
                <div className="mt-2 space-y-1.5">
                  {adminDelhiveryScans.map((phase, index) => (
                    <div key={`${phase.status}-${phase.date}-${index}`} className="rounded-md bg-background/70 p-2">
                      <p className="font-black text-foreground">{phase.status || getFulfillmentStatusLabel(fulfillmentStatus)}</p>
                      <p className="text-muted-foreground">{phase.location || "Location not available"}</p>
                      {phase.date && <p className="text-[10px] font-bold text-muted-foreground">{new Date(phase.date).toLocaleString("en-IN")}</p>}
                    </div>
                  ))}
                </div>
              </div>
            ) : adminSubPhases.length > 0 && (
              <div className="mt-2 rounded-md border border-accent/20 bg-accent/5 p-2">
                <p className="font-black uppercase tracking-widest text-accent">Sub Phase</p>
                {adminSubPhases.map((phase, index) => (
                  <p key={`${phase.status}-${index}`} className="mt-1"><span className="font-black">{phase.status || getFulfillmentStatusLabel(fulfillmentStatus)}</span>{phase.location ? ` - ${phase.location}` : ""}</p>
                ))}
              </div>
            )}
            {order.delhiveryError && <p className="text-red-500">Error: <span className="font-black">{order.delhiveryError}</span></p>}
          </div>
        )}
...
function printOrderInvoice(order: StoreOrder) {
  ...
  const signatureUrl = `${window.location.origin}/brand-signature.png`
      ...
      <head>
        ...
        <style>
          * { box-sizing: border-box; }
          ...
          .signature img { display: block; width: 50mm; max-height: 18mm; margin: 0 auto 0; object-fit: contain; object-position: center bottom; }
          .signature-line { border-top: 1.5px solid #111; margin-top: 0; }
      ...
      <body>
        ...
        <table>
          ...
          <tbody>
            ${order.items.map((item) => `
              <tr>
                <td><strong>${escapeHtml(getShippingLabelItemName(item))}</strong></td>
        ...
        <div class="signature"><img id="terrace-signature" src="${signatureUrl}" alt="terrace.fc authorised signature" /><div class="signature-line"></div><span class="signature-label">Authorised signature</span></div>
        ...
        <script>
          (function () {
            ...
            var signature = document.getElementById("terrace-signature");
            if (!signature || signature.complete) printInvoice();
            else {
              signature.addEventListener("load", printInvoice, { once: true });
              signature.addEventListener("error", printInvoice, { once: true });
              window.setTimeout(printInvoice, 1800);
            }
          })();
        </script>
      </body>
...
function printOrderStickers(orders: StoreOrder[]) {
  const orientation = "horizontal" as const
  const isHorizontal = true
  const labelPages = orders.map((order) => `
    <section class="print-page">
      <div class="print-frame">${stickerHtml(order, orientation)}</div>
    </section>
  `).join("")
  ...
  printWindow.document.write(`
    ...
    <html>
      <head>
        ...
        <style>
          * { box-sizing: border-box; }
          body { margin: 0; padding: 0; font-family: Arial, sans-serif; color: #111; overflow-wrap: anywhere; }
          .sheet { display: block; }
          .print-page { width: ${isHorizontal ? "297mm" : "210mm"}; min-height: ${isHorizontal ? "210mm" : "297mm"}; padding: 10mm; break-after: page; page-break-after: always; }
          .print-frame { width: ${isHorizontal ? "277mm" : "190mm"}; border: 1.5px solid #111; padding: 4.5mm; overflow: visible; display: flex; align-items: flex-start; justify-content: center; }
          .sticker { width: 100%; min-height: 0; border: 0; padding: 0; overflow: visible; display: flex; flex-direction: column; }
          .sticker.horizontal { width: 100%; min-height: 0; max-height: none; }
          ...
          a, .meta, .label, .barcode-text { color: #111; text-decoration: none; }
          .barcode-wrap { width: 62mm; max-width: 100%; margin: 4px 0 0; overflow: visible; }
          .barcode { display: block; width: 62mm; max-width: 100%; height: 13mm; shape-rendering: crispEdges; }
          .barcode-text { margin-top: 1.2mm; font-size: 9px; font-weight: 700; letter-spacing: .04em; text-align: center; line-height: 1; }
          .grid { display: grid; grid-template-columns: ${isHorizontal ? "minmax(0,1.14fr) minmax(0,0.86fr)" : "1fr"}; gap: ${isHorizontal ? "10px" : "0"}; align-items: stretch; }
          .address-column { min-width: 0; padding-right: 2px; }
          .shipment-column { ${isHorizontal ? "border-left: 1px solid #111; margin-bottom: -4.5mm; padding: 0 0 4.5mm 10px;" : "border-top: 1px solid #111; margin-top: 8px; padding-top: 6px;"} }
          .vertical-section { border-top: 1px solid #111; padding: 3px 0; }
          .vertical-split { display: grid; grid-template-columns: minmax(0,1fr) minmax(0,1fr); gap: 5px; align-items: start; }
          .vertical-right { text-align: right; }
          .vertical-right .barcode-text { text-align: center; }
          .vertical-address { min-height: 25mm; }
          .vertical-shipment { min-height: 21mm; }
          .vertical-return { min-height: 30mm; }
          .vertical-items { margin-top: 0; }
          .vertical-item-table { width: 100%; border-collapse: collapse; table-layout: fixed; font-size: 7px; }
          .vertical-item-table th { border-bottom: 1px solid #111; padding: 2px; text-align: left; font-size: 6px; text-transform: uppercase; }
          .vertical-item-table td { border-bottom: 1px solid #aaa; padding: 2px; line-height: 1.15; vertical-align: top; overflow-wrap: break-word; }
          ...
          .label { font-size: 9px; text-transform: uppercase; letter-spacing: .8px; color: #111; margin: 8px 0 3px; font-weight: 900; }
          ...
          .item-number { text-align: right; font-weight: 900; }
          ...
          .summary-row.total-row { margin-top: 3px; border-top: 1px solid #aaa; padding-top: 3px; font-size: 12px; font-weight: 900; }
          ...
          .parcel-value { margin-left: 4px; font-size: 15px; line-height: 1.15; font-weight: 900; }
          .awb { margin-top: 6px; }
  ...
  return `
    <div class="sticker ${orientation}">
      <div class="sticker-head">
        <div class="brand">terrace<span class="brand-dot">.</span>fc</div>
        <strong>${getShippingPaymentLabel(order)}</strong>
      </div>
      <div class="meta">
      ...
      <div class="grid">
        ...
        <section class="shipment-column">
          ...
          <div class="order-barcode">
            ...
            ${barcodeHtml(getShiprocketDisplayOrderId(order))}
          </div>
          <div class="awb">
            <div class="label">AWB: ${escapeHtml(getStickerCourierName(order))}</div>
            ${barcodeHtml(getStickerBarcodeValue(order))}
          </div>
          <div class="items">
            ...
            ${order.items.map((item) => {
              return `<div class="item"><strong>${escapeHtml(getShippingLabelItemName(item))}</strong><span class="item-number">${escapeHtml(item.size)}</span><span class="item-number">${item.quantity}</span><span class="item-number">₹${(getItemPriceWithoutPatches(item) * item.quantity).toLocaleString("en-IN")}</span></div>`
            }).join("")}
  ...
  return `
    <div class="sticker vertical">
      ...
      <section class="vertical-address">
        ...
        <div class="line"><strong>${escapeHtml(order.address.phone)}</strong></div>
        <div class="line">${escapeHtml(order.address.email)}</div>
      </section>
      ...
      <section class="vertical-section vertical-shipment vertical-split">
        <div>
          ...
          <div class="line"><strong>Courier:</strong> ${escapeHtml(getStickerCourierName(order))}</div>
          <div class="line"><strong>Weight:</strong> ${parcelWeightKg.toFixed(2)} kg</div>
          <div class="line"><strong>Pieces:</strong> ${totalUnits}</div>
          <div class="line"><strong>Payment:</strong> ${escapeHtml(getShippingPaymentLabel(order))}</div>
      ...
      <section class="vertical-section vertical-return vertical-split">
        <div>
          ...
          <div class="return-line">Villa 51, R K Garden, Bluejay Malgudi Villas, Sy.No 85, 1st Cross, Opp. Holiday Village Resort, Anjanapura Village, Uttarahalli Hobli, Bengaluru, Karnataka 560109</div>
          <div class="return-line"><strong>Primary:</strong> 8147338142</div>
          <div class="return-line"><strong>Secondary:</strong> 9686616796</div>
          <div class="return-line customer-care">${getShippingCustomerCareHtml(order)}</div>
        </div>
        <div class="vertical-right">
          <div class="label" style="margin-top:0">Order ID: ${escapeHtml(getShiprocketDisplayOrderId(order))}</div>
          ${barcodeHtml(getShiprocketDisplayOrderId(order))}
          <div class="return-line"><strong>Invoice:</strong> ${escapeHtml(getOrderInvoiceNumber(order))}</div>
          <div class="return-line"><strong>Order Date:</strong> ${escapeHtml(formatShiprocketLabelDate(order.shiprocketOrderDate || order.createdAt))}</div>
        </div>
      </section>
      ...
      <div class="items vertical-items">
        ...
        <div class="item-summary">
          <div class="summary-row"><span>Subtotal</span><strong>₹${order.subtotal.toLocaleString("en-IN")}</strong></div>
          <div class="summary-row"><span>Delivery Charges</span><strong>${order.deliveryCharge === 0 ? "FREE" : `₹${order.deliveryCharge.toLocaleString("en-IN")}`}</strong></div>
          ${(order.discount || 0) > 0 ? `<div class="summary-row"><span>Discount${order.couponCode ? ` (${escapeHtml(order.couponCode)})` : ""}</span><strong>-₹${(order.discount || 0).toLocaleString("en-IN")}</strong></div>` : ""}
          <div class="summary-row total-row"><span>Total</span><span>₹${order.total.toLocaleString("en-IN")}</span></div>
        </div>
...
function getOrderPatchSummary(order: StoreOrder) {
  ...
  const types = Array.from(new Set(patchedItems.map((item) => item.customization?.patchType?.trim()).filter(Boolean)))
...
function calculateOrderWeightKg(order: StoreOrder) {
  const totalUnits = order.items.reduce((sum, item) => sum + Math.max(0, item.quantity || 0), 0)
  return Math.max(0.2, Number((totalUnits * 0.35).toFixed(2)))
}
...
function barcodeHtml(value: string) {
  ...
  const encoded = [104, ...codeValues, checksum, 106]
  let x = 2
  const bars = encoded.map((code) => {
    const pattern = CODE128_PATTERNS[code]
    return [...pattern].map((unit, index) => {
      const width = Number(unit)
      const rect = index % 2 === 0 ? `<rect x="${x}" y="0" width="${width}" height="42" fill="#000" />` : ""
      x += width
      return rect
    }).join("")
  }).join("")
  ...
  return `
    <div class="barcode-wrap" aria-label="Order barcode">
      <svg class="barcode" viewBox="0 0 ${x + 2} 42" preserveAspectRatio="xMidYMid meet" role="img" xmlns="http://www.w3.org/2000/svg">
        <rect x="0" y="0" width="${x + 2}" height="42" fill="#fff" />
        ${bars}
      </svg>
