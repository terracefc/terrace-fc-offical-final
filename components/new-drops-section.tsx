  ...
  return (
    <section className="homepage-reveal bg-[#080506] py-10 text-white sm:py-16">
      <div className="px-4 sm:px-8 lg:px-12">
        <p className="text-[10px] font-black uppercase tracking-[0.28em] text-red-300 sm:text-xs sm:tracking-[0.36em]">New Drops</p>
        <h2 className="mt-1 text-2xl font-black uppercase leading-none text-white sm:text-6xl lg:text-7xl">
          {slide.title}
        </h2>
      </div>
        ...
        <div className="relative aspect-[5/4] sm:aspect-[16/10] lg:aspect-video">
          ...
          <div key={slide.image} className="absolute inset-0 new-drop-slide-in">
            <Image
              ...
              sizes="100vw"
              quality={100}
              className="scale-[1.04] object-cover sm:hidden"
              style={{ objectPosition: slide.objectPosition }}
            />
            <Image
              src={slide.image}
          ...
          <div className="relative z-10 flex h-full max-w-7xl flex-col justify-end px-5 pb-8 sm:px-8 sm:pb-12 lg:px-12">
            <p className="mt-3 max-w-xl text-[11px] font-bold leading-relaxed text-white/82 sm:text-base">
              {slide.description}
            </p>
            <Link
              href={slide.href}
              className="mt-5 inline-flex h-10 w-fit items-center gap-2 border border-red-400 bg-red-600 px-3 text-[8px] font-black uppercase tracking-widest text-white transition-colors hover:bg-white hover:text-black sm:mt-6 sm:h-12 sm:px-5 sm:text-xs"
            >
              {slide.cta}
