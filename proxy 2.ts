...
export default clerkMiddleware(async (auth, request: NextRequest) => {
  ...
  if (CUSTOMER_PROTECTED_ROUTES.some((route) => pathname === route || pathname.startsWith(`${route}/`))) {
...
async function getSiteSettings(): Promise<SiteSettings> {
  ...
  try {
    ...
    const settingsUser = data.users?.find((user: any) => user.email === "site-settings@terracefc.local")
    const settings = settingsUser?.user_metadata?.site_settings
...
export const config = {
  matcher: [
    ...
    "/__clerk/:path*",
    "/admin/:path*",
    "/((?!_next/static|_next/image|favicon.ico|icon.svg|icon-light-32x32.png|icon-dark-32x32.png|apple-icon.png|.*\\.(?:png|jpg|jpeg|gif|webp|svg|ico|css|js)$).*)",
  ],
}
