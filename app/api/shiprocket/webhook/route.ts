...
export async function POST(request: Request) {
  ...
  const normalizeMainPhase = (value?: StoreOrder["fulfillmentStatus"]) => value === "picked_up" ? "shipped" : value
  const shouldEmail =
    normalizeMainPhase(order.fulfillmentStatus) !== normalizeMainPhase(fulfillmentStatus) ||
    Boolean(trackingNumber && trackingNumber !== (order.trackingNumber || order.shippingId))
  if (shouldEmail) {
...
function mapShiprocketStatus(status: string, payload: unknown): StoreOrder["fulfillmentStatus"] | null {
  ...
  if (raw.includes("pickup") || raw.includes("picked") || raw.includes("manifested") || raw.includes("awb")) return "shipped"
  return null
}
