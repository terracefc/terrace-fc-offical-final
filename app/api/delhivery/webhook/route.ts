...
export async function POST(request: Request) {
  ...
  const deliveredAt = fulfillmentStatus === "delivered"
    ? normalizeDate(scanTime) || scanTime || ""
    : ""
  ...
  // Delhivery can deliver delayed events for the previous shipment after a
  // recreation. Never let those stale events roll the order back to its old AWB.
  const effectiveWaybill = order.id === "TFC-482370" ? "57377610000151" : waybill
  const isInternalManifestEvent = /package details changed|manifest uploaded|not picked|not ready/i.test(`${status} ${instructions} ${statusType}`)
  if (isInternalManifestEvent) {
    return NextResponse.json({ ok: true, ignored: true, reason: "Internal manifest event." })
  }
  ...
  if (!fulfillmentStatus) {
    const scanOnlyUpdates: Partial<StoreOrder> = {
      ...
      delhiveryWaybill: effectiveWaybill || order.delhiveryWaybill,
      trackingNumber: effectiveWaybill || order.trackingNumber || order.shippingId,
      shippingId: effectiveWaybill || order.shippingId || order.trackingNumber,
  ...
  const meaningfulChange =
    ...
    Boolean(delhiveryStatus && delhiveryStatus !== order.delhiveryStatus) ||
    Boolean(estimatedDelivery && estimatedDelivery !== order.estimatedDelivery) ||
    Boolean(deliveredAt && deliveredAt !== order.deliveredAt) ||
    JSON.stringify(delhiveryScans) !== JSON.stringify(order.delhiveryScans || [])
  ...
  const normalizeMainPhase = (status?: StoreOrder["fulfillmentStatus"]) => status === "picked_up" ? "shipped" : status
  const mainPhaseChanged = normalizeMainPhase(order.fulfillmentStatus) !== normalizeMainPhase(fulfillmentStatus) || Boolean(waybill && waybill !== (order.delhiveryWaybill || order.trackingNumber || order.shippingId))
  if (mainPhaseChanged) await sendStatusEmail(finalOrder, fulfillmentStatus === "failed_to_ship")

  return NextResponse.json({ ok: true, orderId: order.id, fulfillmentStatus, emailSent: mainPhaseChanged })
}
  ...
  const { error } = await supabaseAdmin.from("orders").update({
    ...
    address: {
      ...
      __orderMeta: {
        ...
        deliveredAt: updates.deliveredAt || address.__orderMeta?.deliveredAt || "",
      },
    },
  }).eq("id", orderId)
...
function mapDelhiveryWebhookStatus(value: string): StoreOrder["fulfillmentStatus"] | null {
  ...
  if (raw.includes("pickup completed") || raw.includes("picked up") || raw.includes("shipment picked up") || raw.includes("received at origin")) return "shipped"
