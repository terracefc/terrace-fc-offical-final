...
async function syncDelhiveryOrders(request: Request) {
  ...
  for (const order of delhiveryOrders) {
    ...
    try {
      ...
      // Prefer the latest movement scan over the carrier's high-level
      // manifest label (which can remain "Manifested" after pickup).
      ...
      const changed =
        ...
        Boolean(updates.deliveredAt && updates.deliveredAt !== order.deliveredAt) ||
        JSON.stringify(updates.delhiveryScans || []) !== JSON.stringify(order.delhiveryScans || []) ||
      ...
      // Sub-phase/location scans should update the panels silently. Email only
      // when the main fulfillment phase (or AWB) changes.
      const normalizeMainPhase = (status?: StoreOrder["fulfillmentStatus"]) => status === "picked_up" ? "shipped" : status
      const mainPhaseChanged = normalizeMainPhase(order.fulfillmentStatus) !== normalizeMainPhase(nextStatus) || waybill !== (order.delhiveryWaybill || order.trackingNumber || order.shippingId)
      if (mainPhaseChanged) await sendStatusEmail(finalOrder, nextStatus === "failed_to_ship")
      results.push({ orderId: order.id, waybill, pickupId, changed: true, status: nextStatus, delhiveryStatus: updates.delhiveryStatus, emailSent: mainPhaseChanged })
    } catch (error) {
...
async function readAllOrders() {
  ...
  // If Supabase is unavailable (for example DNS/network outage), continue
  // with the Mongo-backed store so webhook/live tracking data still updates.
  if (error) {
...
function mapDelhiveryStatus(tracking: { status: string; statusType: string; instructions: string; scans?: Array<{ status?: string }> }): StoreOrder["fulfillmentStatus"] | null {
  const scanText = (tracking.scans || []).map((scan) => scan.status || "").join(" ")
  const raw = `${scanText} ${tracking.status} ${tracking.statusType} ${tracking.instructions}`.toLowerCase()
