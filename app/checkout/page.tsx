...
import { isRetroJersey, withDisplayPrice } from "@/lib/pricing"
...
export default function CheckoutPage() {
  ...
  const deliveryCharge = isInternationalDelivery || typeof appliedCoupon?.testTotal === "number"
    ? 0
            ...
            <aside className="rounded-2xl border border-white/10 bg-white/[0.06] p-5 shadow-sm space-y-5">
              ...
              <div className="space-y-4">
                {effectiveCart.map((item) => {
                  ...
                  const isRetroProduct = isRetroJersey(item.kit)
                  ...
                  <div key={item.lineId} className="flex gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {!isRetroProduct && <p className="text-xs text-white/65 truncate">{getJerseyVersionLabel(item.version)}</p>}
                      {!isRetroProduct && item.version !== "embroidery" && (
