...
function OrdersPanel({ customerEmail, orders }: { customerEmail: string; orders: StoreOrder[] }) {
  ...
  const requestCancellation = async (order: StoreOrder) => {
    ...
    if (!reason) {
      setCancellationMessages((current) => ({ ...current, [order.id]: "Please add a reason for the cancellation request." }))
      return
    }
          ...
          <div className="mt-5 space-y-2">
            {order.items.map((item) => {
              ...
              <div key={`${order.id}-${item.id}-${item.size}-${item.version || "plain"}-${customization.name}-${customization.number}`} className="flex justify-between gap-3 text-sm">
                <span>
                  {item.name} - {item.club} - Size {item.size} - {getJerseyVersionLabel(item.version)}
                  {item.version !== "embroidery" ? ` - ${isCustomBack(customization) ? `Name: ${customization.name.trim() || "Name"}, Number: ${customization.number.trim() || "00"}` : getJerseyBackLabel(customization)}` : ""} x{item.quantity}
                </span>
